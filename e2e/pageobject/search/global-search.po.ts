...
class GlobalSearch {
    ...
    async selectTime(createdDateOrModifyDate: string, startEnd: string, hours: string, seconds: string, amPM: string): Promise<void> {

        if (startEnd == 'Start') {
            await $(this.selectors.advanceFilterButton).click();
            await element(by.cssContainingText('.card-title-text .ellipsis', createdDateOrModifyDate)).click();
    
            for (let a = 1; a <= 12; a++) {
                let getHours = await $$('.a3t-datetime--time-picker-summary-input').get(0).getText();

                if (getHours.includes(hours)) {
                    break;
                } else {
                    await $$('.a3t-datetime--time-picker-summary-input').get(0).click();
                    await $$('.a3t-datetime--time-picker-summary-input').get(0).sendKeys(protractor.Key.ARROW_DOWN);
                }

                for (let b = 0; b < 60; b++) {
                    let getSecond = await $$('.a3t-datetime--time-picker-summary-input').get(1).getText();

                    if (getSecond == seconds) {
                        break;
                    } else {
                        await $$('.a3t-datetime--time-picker-summary-input').get(1).click();
                        await $$('.a3t-datetime--time-picker-summary-input').get(1).sendKeys(protractor.Key.ARROW_DOWN);
                    }

                    let getAmPm = await $$('.a3t-datetime--time-picker-summary-input').get(2).getText();

                    if (getAmPm.includes(amPM)) {
                    } else {
                        await $$('.a3t-datetime--time-picker-summary-input').get(2).click();
                        await $$('.a3t-datetime--time-picker-summary-input').get(2).sendKeys(protractor.Key.ARROW_DOWN);
                    }
                }
            }
            ...
            await $$('.a3t-datetime-range--header button').get(1).click();
            for (let a = 1; a <= 12; a++) {
                let getHours = await $$('.a3t-datetime--time-picker-summary-input').get(0).getText();
