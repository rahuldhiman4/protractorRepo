...
describe('AcknowledgmentTemplate', () => {
    ...
    fit('DRDMV-10902 : Acknowledgment Template: Acknowledgment Template creation with same name', async () => {
        ...
        await utilCommon.waitUntilSpinnerToHide();
        await editAcknowledgmentTemplatePo.updateTemplateName(templateName);
        ...
        await editAcknowledgmentTemplatePo.updateDescription(description);
        ...
        expect(await editAcknowledgmentTemplatePo.isLocalizedMessageButtonDisplayed()).toBeTruthy('Localize Message button is missing');
        expect(await editAcknowledgmentTemplatePo.getSelectedGridRecordValue('Message Type')).toBe('body', 'Body is missing from Grid');
        await editAcknowledgmentTemplatePo.searchOnGridConsole('subject');
        expect(await editAcknowledgmentTemplatePo.getSelectedGridRecordValue('Message Type')).toBe('subject', 'subject is missing from Grid');
        ...
        await consoleAcknowledgmentTemplatePo.addFilter('Template Name', templateName, 'text');
        expect(await consoleAcknowledgmentTemplatePo.getSelectedGridRecordValue('Template Name')).toBe(templateName, 'Filter Template Name is missing in column');
        ...
        await consoleAcknowledgmentTemplatePo.addFilter('Subject', subject, 'text');
        expect(await consoleAcknowledgmentTemplatePo.getSelectedGridRecordValue('Subject')).toBe(subject, 'Filter Subject is missing in column');
        await consoleAcknowledgmentTemplatePo.removeColumnOnGrid(arr2);
        await consoleAcknowledgmentTemplatePo.clearGridFilter();

        await consoleAcknowledgmentTemplatePo.searchOnGridConsole(templateName);
        expect(await consoleAcknowledgmentTemplatePo.getSelectedGridRecordValue('Template Name')).toBe(templateName, 'Search Template Name is missing in column');
        ...
        await createAcknowledgmentTemplatesPo.setBody(body);
        await createAcknowledgmentTemplatesPo.clickOnSaveButton();
        expect(await utilCommon.getPopUpMessage()).toBe('ERROR (222108): Template Already exist with given name:'+templateName,'Duplicate private template name error message is missing');
